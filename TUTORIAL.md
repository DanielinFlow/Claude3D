# Claude3D + OpenClaw + Tailscale Setup Tutorial

This guide is a step-by-step runbook for the most common production-like setup:

- **Machine A** runs **OpenClaw Gateway**.
- **Machine B** runs **Claude3D**.
- **Tailscale** connects both machines securely.

If you follow this exactly, people should avoid the most common confusion: **Claude3D does not install or run OpenClaw for you.**
...
- **Claude3D** is the UI and Studio proxy.
- Claude3D connects to an already running OpenClaw Gateway.
- In this tutorial, the Gateway lives on a different machine from Claude3D.
...
You will need this token in Claude3D:
...
This host is what Claude3D will use as `wss://<gateway-host>.<tailnet>.ts.net`.

---

## 5) Install and Run Claude3D on Machine B
...
## 6) Connect Claude3D to OpenClaw

In Claude3D connection UI:
...
- Use `ws://localhost:18789` only when Gateway is local to the same machine as Claude3D or when using an SSH tunnel.
...
After Claude3D is running and tries to connect for the first time, approve pending device pairing on **Machine A**:
...
4. Claude3D connect UI uses `wss://...ts.net` plus valid token.
5. `openclaw devices approve --latest` has been run after first connect attempt.
6. Claude3D UI shows gateway connected and loads agents.
...
### `401` or auth errors from Claude3D

- Re-copy token from Machine A:
  - `openclaw config get gateway.auth.token`.
- Confirm Gateway auth mode and token are current.

### Claude3D still cannot connect after token is correct
