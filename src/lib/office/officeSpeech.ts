/**
 * Office feed entries for turns driven from outside this Studio.
 *
 * A gateway sends message events only to the client that started the prompt,
 * so chatting in the desktop app, the TUI, or the CLI leaves the office
 * blind. A gateway-side publisher (an OpenClaw plugin) can republish each
 * finished turn as an `office.speech` gateway event, and this turns that
 * event into the same kind of feed entry a locally driven reply produces.
 *
