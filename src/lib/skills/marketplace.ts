...
> = {
  ...
  "todo-board": {
    ...
    featured: true,
    editorBadge: "Claude3D test",
    hideStats: true,
  },
  "task-manager": {
    category: "Productivity",
    tagline:
      "Turns actionable requests into persistent shared tasks that power the Claude3D Kanban board.",
    capabilities: [
...
export const buildSkillMarketplaceEntry = (
  ...
  if (packagedSkill && !skill.baseDir.trim()) {
    missingDetails.unshift(
      "Install this packaged Claude3D skill to make it available on the gateway.",
    );
  }
  return {
    skill,
...
export const buildSkillMarketplaceCollections = (
  ...
  if (claw3d.length > 0) {
    collections.push({ id: "claw3d", label: "Claude3D", entries: claw3d });
  }
