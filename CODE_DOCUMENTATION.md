...
This file is the practical code map for Claude3D contributors.
...
Claude3D is the UI and local Studio/proxy layer around an existing OpenClaw Gateway.

- OpenClaw owns agent execution, sessions, tools, config, and runtime events.
- Claude3D owns visualization, local Studio settings, UI workflows, office rendering, and the same-origin WebSocket/API bridge.
...
Sometimes Claude3D behavior depends on the upstream event contract or session behavior. In those cases:
...
3. Apply changes in Claude3D unless the user explicitly asked for upstream OpenClaw work.
