...
## Overview
Claude3D is a gateway-first Next.js application for visualizing and operating AI agents powered by OpenClaw using Three.JS framework.

It is the UI and proxy layer, not the OpenClaw runtime itself. OpenClaw remains the system of record for agents, sessions, and execution, while Claude3D provides:
...
## System Model
Claude3D is split into four main parts:
...
Claude3D may read and mutate that state through gateway APIs, but it should not create a competing local source of truth.
...
- Agent configuration and files are managed through gateway APIs.
  This avoids drift between Claude3D and the upstream runtime.
