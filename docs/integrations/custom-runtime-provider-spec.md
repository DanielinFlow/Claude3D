...
Define a clean `custom` runtime provider class for Claude3D.

This provider should let external orchestration stacks integrate with Claude3D through a stable seam without requiring:
...
- Claude3D probes `GET /health`, `GET /state`, and `GET /registry`
- chat uses a direct HTTP path to `POST /v1/chat/completions`
- browser traffic is proxied through Claude3D's same-origin
...
Not every useful runtime should have to become a named built-in provider in upstream Claude3D.
...
Claude3D’s runtime abstraction already points toward multiple providers:
...
It should not require upstream Claude3D to know:
...
This gives Claude3D enough identity for UI without hardcoding a brand into the provider class.
...
This keeps Claude3D stable even if the custom stack has richer private event semantics internally.
...
The custom provider is the right place for richer internal state to enter Claude3D.
...
The custom provider should be able to support office systems without Claude3D needing to know the backend’s internals.
...
- make no runtime-specific assumptions in core Claude3D

Actual runtime adapters can be implemented separately against that seam.

## Out of Scope For V1

- embedding proprietary internal orchestration logic in Claude3D core
...
- upstream Claude3D gains a clean extension seam
...
It gives Claude3D extensibility, gives Vera a clean path in, and avoids hardwiring a personal stack directly into the core product model.
