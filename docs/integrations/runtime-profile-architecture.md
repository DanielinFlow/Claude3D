...
> Forward-looking runtime model for Claude3D after the OpenClaw + Hermes adapter work landed on `main`.

## Goal

Claude3D should treat runtime connection targets as profiles, not as ad hoc
...
- the UI keeps speaking one Claude3D gateway contract
...
- if nothing else is configured, Claude3D prefers `OpenClaw`
...
This avoids making the "Custom Floor" logic one-off and lets Claude3D grow
...
- the browser talks one Claude3D gateway contract
...
Claude3D should move to a runtime profile model where:
