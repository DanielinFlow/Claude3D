...
> Reference implementation guide for plugging a non-OpenClaw, non-Hermes runtime into Claude3D through the upstream-safe `custom` provider seam.

## Goal

Show how a custom orchestration stack should plug into Claude3D without requiring:
...
A custom runtime should sit at the Claude3D boundary as an orchestrator-backed service.

That means:

- Claude3D should talk to one stable runtime boundary
...
The `custom` provider exists so those stacks can integrate cleanly without forcing Claude3D core to absorb stack-specific assumptions.

## Recommended Boundary

Claude3D should integrate with the custom runtime's orchestrator or gateway layer, not with its individual workers.
...
This gives the UI enough identity without forcing upstream Claude3D to branch on every runtime brand.
...
Do not claim support just because the backend can theoretically do something. Claim support only where the provider exposes a stable Claude3D-facing behavior.

## Agent Mapping

Claude3D agents should not be modeled as one fixed backend process each unless the runtime truly behaves that way.
...
Claude3D sessions should map to runtime conversations or execution threads, not to whichever backend storage structure happens to exist internally.
...
The provider should translate runtime activity into the same normalized Claude3D runtime events used elsewhere.
...
The custom provider is the right place to map richer internal runtime signals into Claude3D's public office state model.
...
Claude3D only needs the resulting office-safe state and maybe a public reason label.

## Relationship To Office Systems

The custom provider should support office systems without Claude3D needing to know the backend's internals.
...
3. Map runtime roles or routes into Claude3D agent identities.
...
This is enough to make a custom runtime useful in Claude3D without overexposing internals.
...
- runtime-specific branches throughout Claude3D core
