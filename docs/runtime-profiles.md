...
Claude3D now treats runtime backends as named saved profiles instead of one global URL/token pair.
...
This is the provider-rich path. OpenClaw already knows how to sit in front of many upstream model providers, so Claude3D should treat it as a first-class gateway adapter rather than flattening it into `custom`.
...
A Claude3D-shaped HTTP runtime profile for stacks that want to keep Claude3D transcript and chat conventions while still using the direct runtime seam.
...
The generic HTTP runtime seam when you want to point Claude3D at any compatible orchestrator boundary.
...
The current provider review path should borrow from existing Hermes/OpenClaw wizard flows where possible, but land as Claude3D-native adapters instead of hard-coupling Claude3D UI state to another project's connector code.
