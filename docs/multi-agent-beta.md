...
This document explains the current multi-agent beta in Claude3D: what it does, how the two connection modes work, and how to connect a second office.

## What This Beta Does

Claude3D can render a second office inside the same 3D scene so you can visualize agents from another machine.
...
- **Local office**: the Claude3D instance you are currently using;
- **Remote office**: another Claude3D instance or another OpenClaw gateway you want to visualize.

The remote office can be connected in one of two ways:

1. **Remote Claude3D presence endpoint**.
...
Use this when the other machine is also running Claude3D.

How it works:

- your local Claude3D server polls the remote Claude3D `presence` endpoint;
...
This mode is best when you want the remote side to feel like another full Claude3D office.

### 2. Remote OpenClaw Gateway

Use this when the other machine only runs OpenClaw and does not run Claude3D.

How it works:

- the browser connects directly to the remote gateway;
- Claude3D derives a read-only presence snapshot from gateway data such as `agents.list`, `status`, and `sessions.preview`;
- because there is no remote Claude3D layout endpoint, the second office uses a fallback office visualization.
...
If you paste an `http://` or `https://` URL into gateway mode, Claude3D normalizes it to `ws://` or `wss://` before connecting.

This mode is best when you want remote agent visibility without requiring a second Claude3D deployment.
...
- is designed to avoid exposing remote files or tool output in the Claude3D UI.
...
Under the hood, Claude3D now uses a shared runtime contract for:
...
- your local Claude3D is already working with your local OpenClaw gateway;
...
1. Start your local Claude3D instance.
...
### Setup For `Remote Claude3D presence endpoint`

Use:

- `Source type`: `Remote Claude3D presence endpoint`.
- `Presence URL`: the remote `/api/office/presence` URL.
- `Optional token`: only if that remote Claude3D endpoint is protected.
...
- if the remote layout snapshot is unavailable, Claude3D falls back to a default/fallback office rendering for the remote side.
...
- the office shell is a fallback visualization, not a true remote layout clone from another Claude3D instance.

## Recommended Network Patterns

### Same private network

Use a reachable private IP or local hostname for the remote Claude3D endpoint or OpenClaw gateway.
...
- remote Claude3D endpoint over `https://<machine>.ts.net/api/office/presence`;
...
That usually means the other machine has Claude3D presence available but not a layout snapshot yet. The beta should still render a fallback remote office.
...
- Gateway mode derives presence from gateway snapshots rather than a real remote Claude3D layout.
...
Use `Remote Claude3D presence endpoint` when the other side runs Claude3D and you want the most complete office visualization.
